...
import { getContent, getDocumentTitle, getReadingMinutes, getWordCount, UNASSIGNED_SPACE, type FlowDocument } from "./components/library/types";
...
function App() {
  ...
  const saveDraft = () => {
    ...
    const now = Date.now();
    if (activeDocumentId) updateDocument(activeDocumentId, { content: text, title: draftTitle.trim() || "Untitled note", tags: draftTags, updatedAt: now, wordCount: getWordCount(text), readingTime: getReadingMinutes(text) });
    else { const document = createDocument(text, activeSpace ?? "personal"); setActiveDocumentId(document.id); updateDocument(document.id, { title: draftTitle.trim() || "Untitled note", tags: draftTags }); }
    setNotice("Saved to your writing library.");
  };
  const saveAndNew = () => { if (text.trim()) saveDraft(); const targetSpace = activeSpace; if (targetSpace) openNewDocument(targetSpace); else setShowSpacePicker(true); };
  const openNewDocument = (space: string) => { setActiveDocumentId(null); setDraftTitle(""); setDraftTags([]); setActiveSpace(space); setText(""); setSuggestion(""); setNotice("Fresh page, ready when you are."); setShowSpacePicker(false); setTab("write"); window.setTimeout(() => textareaRef.current?.focus(), 0); };
