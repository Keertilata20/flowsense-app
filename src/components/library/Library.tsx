...
import { UNASSIGNED_SPACE, type FlowDocument, type SortOption, type SpaceDefinition } from "./types";
...
export default function Library({ documents, spaces, onOpen, onRename, onDelete, onFavorite, onMove, onStartWriting, onCreateSpace, onSpaceChange }: LibraryProps) {
  ...
  const availableSpaces = [...spaces, UNASSIGNED_SPACE];
  ...
  const section = (title: string, items: FlowDocument[]) => items.length > 0 && <section className="mb-10"><div className="mb-3 flex items-baseline gap-3"><h2 className="font-['Instrument_Serif',Georgia,serif] text-2xl font-normal tracking-[-0.02em] text-[#493a31]">{title}</h2><span className="text-xs text-[#9a8a7e]">{items.length}</span></div><div className="grid gap-3 md:grid-cols-2">{items.map((document) => <DocumentCard key={`${title}-${document.id}`} document={document} spaces={availableSpaces} onOpen={onOpen} onRename={onRename} onDelete={onDelete} onFavorite={onFavorite} onMove={onMove} />)}</div></section>;
  return <main className="mx-auto w-full max-w-[1120px] px-5 pb-20 pt-10 sm:px-8 sm:pt-14"><LibraryHeader count={filtered.length} search={search} sort={sort} space={space} spaces={availableSpaces} onSearchChange={setSearch} onSortChange={setSort} onSpaceChange={setSpace} onCreateSpace={onCreateSpace} />{filtered.length ? <>{section("Favorites", favorites)}{section("Recent Documents", recent)}</> : <EmptyLibrary isSearchEmpty={Boolean(search.trim())} onStartWriting={() => onStartWriting(space === "all" ? undefined : space)} />}</main>;
}
