...
export default function RichFormattingToolbar() {
  ...
  return <div className="flex flex-wrap items-center gap-0.5 border-b border-[#ece2d8] px-5 py-2 sm:px-8" aria-label="Formatting toolbar">{buttons.map(([label, icon, action]) => <button key={label} type="button" title={label} aria-label={label} onMouseDown={(event) => event.preventDefault()} onClick={action} className="grid h-8 w-8 place-items-center rounded-md text-[#806f63] transition hover:bg-[#f7eee6] hover:text-[#4f3c31]">{icon}</button>)}<span className="mx-1 h-5 w-px bg-[#e5d9cf]" /><select aria-label="Font family" title="Font family" defaultValue="" onMouseDown={(event) => event.preventDefault()} onChange={(event) => runCommand("fontName", event.target.value)} className="h-8 rounded-md bg-transparent px-1 text-xs text-[#75665c] outline-none hover:bg-[#f7eee6]"><option value="">Font</option><option value="Georgia">Serif</option><option value="Inter">Sans</option><option value="Courier New">Mono</option></select><select aria-label="Font size" title="Font size" defaultValue="" onMouseDown={(event) => event.preventDefault()} onChange={(event) => runCommand("fontSize", event.target.value)} className="h-8 rounded-md bg-transparent px-1 text-xs text-[#75665c] outline-none hover:bg-[#f7eee6]"><option value="">Size</option><option value="2">Small</option><option value="3">Normal</option><option value="4">Large</option><option value="5">XL</option></select></div>;
}
